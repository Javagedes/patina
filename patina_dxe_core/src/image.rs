...
// Compile time assert to make sure `STACK_ALIGNEMENT` (which comes from uefi_corosensei) is never larger than
...
impl PrivateImageData {
    ...
    fn load_resource_section(&mut self, image: &[u8]) -> Result<(), EfiError> {
        ...
        let result = pecoff::load_resource_section(&self.pe_info, image)
            .inspect_err(|err| {
    ...
    fn relocate_image(&mut self) -> Result<(), EfiError> {
        ...
        self.relocation_data =
            pecoff::relocate_image(&self.pe_info, physical_addr, image_buffer, &self.relocation_data)
                .inspect_err(|err| {
                    let pe_file_name = self.pe_info.filename_or("Unknown");
    ...
    /// Installs all necessary protocols for the image on the specified handle.
    fn install(&self) -> Result<efi::Handle, EfiError> {
        // install the loaded_image protocol
        ...
        // install the loaded_image device path protocol
        ...
        // install the HII Resource Section protocol
        ...
        // if this is a runtime driver, register it with the runtime services
    ...
    fn uninstall(&self, handle: efi::Handle) -> Result<(), EfiError> {
        ...
        {
            log::warn!("Failed to uninstall loaded image device path for handle {handle:?}: {err:?}");
            result = Err(err);
        }
        ...
        if self.pe_info.image_type == EFI_IMAGE_SUBSYSTEM_EFI_RUNTIME_DRIVER
            ...
            && err != EfiError::NotFound
        {
            log::error!("Failed to remove runtime image for handle {handle:?}: {err:?}");
            return Ok(());
        }
