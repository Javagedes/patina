...
use crate::{
    acpi_table::{AcpiDsdt, AcpiFacs, AcpiFadt, AcpiRsdp, GenericTable, AcpiTableHeader, AcpiXsdt, AcpiXsdtMetadata},
    alloc::{vec, vec::Vec},
    error::AcpiError,
    hob::AcpiMemoryHob,
...
pub(crate) struct StandardAcpiProvider<B: BootServices + 'static> {
    ...
    pub(crate) acpi_tables: TplMutex<BTreeMap<TableKey, GenericTable>, B>,
...
struct StandardAcpiProvider2<B: BootServices> {
    critical_section: TplMutex<(), B>,
    inner: UnsafeCell<InnerAcpiProvider<B>>,
}

impl<B: BootServices + Clone> StandardAcpiProvider2<B> {
...
impl<B: BootServices> StandardAcpiProvider2<B> {
    fn critical_section<F, R>(&self, f: F) -> R
    where
        F: FnOnce(&mut InnerAcpiProvider<B>) -> R,
    {
        let _guard = self.critical_section.lock();
        // SAFETY: We hold the critical section lock, so it is safe to access `inner`.
    ...
    pub fn new_uninit() -> Self {
        Self {
            ...
            inner: UnsafeCell::new(InnerAcpiProvider {
...
impl<B: BootServices>AcpiProvider for StandardAcpiProvider2<B> {
    fn install_acpi_table(&self, table: GenericTable) -> Result<TableKey, AcpiError> {
        self.critical_section(|inner| inner.install_acpi_table(table))
    }

    fn uninstall_acpi_table(&self, key: TableKey) -> Result<(), AcpiError> {
        self.critical_section(|inner| inner.uninstall_acpi_table(key))
    }

    fn get_acpi_table(&self,table_key:TableKey) -> Result<GenericTable,AcpiError> {
    ...
    fn collect_tables(&self) -> Vec<GenericTable> {
        self.critical_section(|inner| inner.collect_tables())
    }
}

struct InnerAcpiProvider<B: BootServices> {
    acpi_tables: BTreeMap<TableKey, GenericTable>,
...
impl <B: BootServices> InnerAcpiProvider<B> {
    ...
    pub fn initialize(&mut self, bs: B, memory_manager: Service<dyn MemoryManager>) -> Result<(), AcpiError> {
        if self.boot_services.is_some() {
    ...
    fn set_rsdp(&mut self, rsdp: &'static mut AcpiRsdp) {
        self.rsdp = Some(rsdp);
    }

    fn set_xsdt(&mut self, xsdt_data: AcpiXsdtMetadata) {
        self.xsdt_metadata = Some(xsdt_data);
    }

    fn install_acpi_table(&mut self, table: GenericTable) -> Result<TableKey, AcpiError> {
        ...
        let table_key = match table.signature() {
            ...
            signature::DSDT => self.install_fadt(table)?,
            signature::FADT => self.install_dsdt(table)?,
            _ => self.install_table(table)?,
        };
        
        self.publish_tables()?;
        self.notify_acpi_list(table_key)?;
        log::trace!("ACPI table {:#?} installed with key {:?}", table.signature(), table_key);
        Ok(table_key)
    }

    fn uninstall_acpi_table(&mut self, key: TableKey) -> Result<(), AcpiError> {
        todo!()
    }

    fn get_acpi_table(&mut self, table_key: TableKey) -> Result<GenericTable, AcpiError> {
        self.acpi_tables.get(&table_key).cloned().ok_or(AcpiError::TableNotPresentInMemory)
    }
    ...
    fn collect_tables(&self) -> Vec<GenericTable> {
        self.acpi_tables.values().cloned().collect()
    }

    fn install_facs(&mut self, facs_info: GenericTable) -> Result<TableKey, AcpiError> {
    ...
    B: BootServices,
{
    ...
    fn collect_tables(&self) -> Vec<GenericTable> {
        todo!() // self.acpi_tables.lock().values().cloned().collect()
    }
}
    ...
    B: BootServices,
{
    ...
    pub(crate) unsafe fn install_facs(&self, facs_info: GenericTable) -> Result<TableKey, AcpiError> {
    ...
    pub(crate) fn install_fadt(&self, mut fadt_info: GenericTable) -> Result<TableKey, AcpiError> {
    ...
    pub(crate) fn install_dsdt(&self, mut dsdt_info: GenericTable) -> Result<TableKey, AcpiError> {
    ...
    pub(crate) fn install_standard_table(&self, mut table_info: GenericTable) -> Result<TableKey, AcpiError> {
    ...
    pub(crate) fn get_table_at_idx(&self, idx: usize) -> Result<(TableKey, GenericTable), AcpiError> {
        todo!()
