...
use crate::{
    acpi_table::GenericTable,
    error::AcpiError,
};

pub use crate::acpi_table::AcpiTableHeader;
...
/// ## Safety
/// 
/// - The first field of the provided table must be [AcpiTableHeader].
pub unsafe trait AcpiTable: zerocopy::FromBytes + zerocopy::IntoBytes + zerocopy::Immutable + zerocopy::KnownLayout {
    fn header(&self) -> &AcpiTableHeader {
        <AcpiTableHeader as zerocopy::FromBytes>::ref_from_prefix(self.as_bytes()).unwrap().0
    }
...
impl AcpiTableManager {
    ...
    // pub unsafe fn install_acpi_table<T: Table>(&self, table: T) -> Result<TableKey, AcpiError> {
    ...
    pub fn iter_tables(&self) -> Vec<GenericTable> {
...
pub(crate) trait AcpiProvider {
    ...
    fn install_acpi_table(&self, acpi_table: GenericTable) -> Result<TableKey, AcpiError>;
