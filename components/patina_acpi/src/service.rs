...
pub unsafe trait AcpiTable: zerocopy::FromBytes + zerocopy::IntoBytes + zerocopy::Immutable + zerocopy::KnownLayout {
    fn header(&self) -> &AcpiTableHeader {
        <AcpiTableHeader as zerocopy::FromBytes>::ref_from_prefix(self.as_bytes()).expect("Trait ensures the first field is an AcpiTableHeader").0
    }
...
mod tests {
    ...
    fn test_get_table_wrong_type() {
        ...
        mock_acpi_provider.expect_get_acpi_table().returning(move |_| table());
