...
use patina_smbios::{
    service::{SMBIOS_HANDLE_PI_RESERVED, Smbios, SmbiosTableHeader},
    smbios_record::{
...
///
/// This example shows:
/// - Using the `#[derive(SmbiosRecord)]` macro for automatic helper methods
/// - Defining a custom struct with SmbiosTableHeader
/// - Using a custom type code (0x80) in the vendor range
/// - Adding custom fields and string pools with `#[string_pool]` attribute
///
...
impl SmbiosExampleComponent {
    ...
    fn entry_point(self, smbios: Service<Smbios>) -> Result<()> {
