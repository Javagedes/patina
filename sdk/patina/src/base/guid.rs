...
mod tests {
    ...
    // #[test]
    // fn test_parse_hexs() {
    //     // The macro generates `return Err(...)` on failure, so we need to wrap it in a closure that can catch those
    //     // returns, because we can't have early returns in tests.

    //     let parse_32 = || -> Result<u32, GuidError> {
    //         let string = "550e8400";
    //         let bytes = string.as_bytes();
    //         Ok(parse_hex!(bytes, 0, 8, u32))
    //     };

    //     assert_eq!(parse_32(), Ok(0x550e8400));

    //     let parse_16 = || -> Result<u16, GuidError> {
    //         let string = "e29b";
    //         let bytes = string.as_bytes();
    //         Ok(parse_hex!(bytes, 0, 4, u16))
    //     };

    //     assert_eq!(parse_16(), Ok(0xe29b));

    //     let parse_8 = || -> Result<u8, GuidError> {
    //         let string = "a7";
    //         let bytes = string.as_bytes();
    //         Ok(parse_hex!(bytes, 0, 2, u8))
    //     };

    //     assert_eq!(parse_8(), Ok(0xa7));

    //     let parse_invalid_hex = || -> Result<u8, GuidError> {
    //         let string = "zz";
    //         let bytes = string.as_bytes();
    //         Ok(parse_hex!(bytes, 0, 2, u8))
    //     };

    //     assert_eq!(parse_invalid_hex(), Err(GuidError::InvalidHexCharacter { position: 0, character: 'z' }));

    //     let parse_invalid_hex_with_weird_length = || -> Result<u8, GuidError> {
    //         let string = "1234567890abcdefz";
    //         let bytes = string.as_bytes();
    //         Ok(parse_hex!(bytes, 15, 2, u8))
    //     };

    //     assert_eq!(
    //         parse_invalid_hex_with_weird_length(),
    //         Err(GuidError::InvalidHexCharacter { position: 16, character: 'z' })
    //     );
    // }
}
